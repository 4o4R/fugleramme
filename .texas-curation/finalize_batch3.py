...
ATTRIBUTION_SNIPPETS = {
    ...
    'ann-lee-ramm': "\n\n**Ann Lee (RAMM)** - *Yellow warbler* (**Setophaga petechia**), a 1770 illustration by **Ann Lee** in the collection of the **Royal Albert Memorial Museum & Art Gallery**, via the Wikimedia Commons file page linked in the manifest. Public domain. Manifest key: `ann-lee-ramm`.\n",
}
...
def main() -> None:
    ...
    repo = args.repo.resolve(); candidates = args.candidates.resolve(); output = args.output.resolve()
    ...
    for item in approved:
        ...
        shipped.append({
            'common': row['common'], 'scientific': row['scientific'], 'key': key, 'asset': dest.name,
            'source_key': item['source_key'], 'source_page': row['source_page'], 'source_title': row['source_title'],
            'artist': row['artist'], 'credit': row['credit'], 'license': row['license'],
            'mask_index': row['mask_index'], 'review_status': 'approved',
        })
    ...
    for source_key in sorted({r['source_key'] for r in shipped}):
        ...
        if marker not in attribution:
            attribution += ATTRIBUTION_SNIPPETS[source_key]
    ...
    (output / 'summary.json').write_text(json.dumps({
        'approved_requested': len(approved), 'shipped': len(shipped),
        'skipped_existing': len(approved) - len(shipped),
    }, indent=2) + '\n')
    if not shipped:
